...
public class ContainerProperties extends PropertiesHelper {
	 ...
	 * (for example "http://localhost:7777/"). 	If using a gateway, this will be the 
	 * gateway address with the sitename appended. 
	 *  
	 */
	...
	static
	{
		...
		META.put(VSITE_NAME_PROPERTY, new PropertyMD("DEMO-SITE").setDescription(
				"Short, human friendly, name of the target system, should be unique in the grid."));
		...
		META.put(WSRF_HOST, new PropertyMD("localhost").
				setDescription("Server interface to listen on."));
		META.put(WSRF_PORT, new PropertyMD("7777").setBounds(0, 65535).
	...
	private void updatePropertiesInternal(boolean sslMode) {
		if (!isSet(WSRF_BASEURL)) {
			String host = getValue(WSRF_HOST);
			if("0.0.0.0".equals(host)) {
				try{
					host = InetAddress.getLocalHost().getCanonicalHostName();
				}catch(Exception ex) {
					throw new ConfigurationException("Property <"+PREFIX+WSRF_BASEURL+"> is not set.");
				}
			}
			String port = getValue(WSRF_PORT);
			String path = getValue(WSRF_SERVLETPATH);
			String protocol = sslMode ? "https" : "http";
			String baseUrl = protocol + "://" + host + ":" + port + path;
			setProperty(WSRF_BASEURL, baseUrl);
			log.info("Using an autogenerated baseurl = "+baseUrl);
		}

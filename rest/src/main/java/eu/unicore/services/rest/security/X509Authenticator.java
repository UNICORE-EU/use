...
public class X509Authenticator implements IAuthenticator, KernelInjectable  {
	...
	private boolean extractFromTLS(Message message, SecurityTokens tokens){
	...
	private boolean extractFromGWHeader(Message message, SecurityTokens tokens){
		HttpServletRequest req =(HttpServletRequest)message.get(AbstractHTTPDestination.HTTP_REQUEST);
		if (req == null)return false;
		String consignerInfo = req.getHeader("X-UNICORE-Consignor");
		if(consignerInfo==null)return false;
		return checkConsignerInfo(consignerInfo, tokens);
	}

	boolean checkConsignerInfo(String consignerInfo, SecurityTokens tokens){
		String dn = null;
		String dsig = null;

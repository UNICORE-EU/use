...
/**
 * Handles the client-to-backend part of the forwarding for all running forwarding connections
 *
 * @author schuller
 */
public class Forwarder implements Runnable {
	...
	public synchronized void attach(final SocketChannel backend, final ForwardingConnection toClient) 
	throws IOException {
		assert toClient!=null : "Client connection cannot be null";
		assert backend!=null : "Backend application socket channel cannot be null";
		backend.configureBlocking(false);
		SelectionKey key  = backend.register(selector,
